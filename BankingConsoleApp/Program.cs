...
// ----------------------------------- Account Opening. --------------------------------
    ...
    private void CalculateInterest()
    {
        ...
        {
            var monthsSinceLastCalculation = (DateTime.Now - account.LastInterestCalculation).Days / 30;

            if (monthsSinceLastCalculation >= 1)
            {
                decimal interest = account.Balance * (INTEREST_RATE / 12) * monthsSinceLastCalculation;
            ...
            {
                Console.WriteLine($"No interest due for account {account.AccountNumber}");
            }
        }
